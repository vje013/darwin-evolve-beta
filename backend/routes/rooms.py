...
async def get_live_models(user: dict = Depends(get_current_user)):
    ...
    if _models_cache["data"] and (now - _models_cache["fetched_at"]) < 86400:
        filtered = list(_models_cache["data"])
    else:
        try:
    ...
    # Add custom trained model if available
    from db import get_db as _get_db
    with _get_db() as conn:
        trained = conn.execute(
            "SELECT model_id, training_examples, final_loss, created_at FROM trained_models WHERE status='ready' ORDER BY created_at DESC LIMIT 1"
        ).fetchone()
    if trained:
        filtered.insert(0, {
            "id": "custom/trained",
            "name": "Custom (Darwin Trained)",
            "description": f"LoRA fine-tuned on {trained['training_examples']} examples from your team's knowledge graph. Loss: {trained['final_loss']:.4f}",
            "context_length": 4096,
